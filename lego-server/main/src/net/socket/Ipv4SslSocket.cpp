...
Ipv4SslSocket::Ipv4SslSocket(std::string certFilePath, int port) {
  ...
  try {
    this->sslContext.setCtx(SSL_CTX_new(TLS_client_method()));
    this->SSLLoadCertificates(certFilePath.c_str(), certFilePath.c_str());
    this->sslBind(port);
...
void Ipv4SslSocket::Connect(const char *host, int port) {
  ...
  if (status == 0) {
    throw runtime_error(appendSslErr("Invalid IPv4 address Socket::Connect"));
  } else if (status == -1) {
    throw runtime_error(
        appendSslErr("Error converting IPv4 address Socket::Connect"));
  }
...
void Ipv4SslSocket::sslAccept() noexcept(false) {
  while (true) {
    ...
    std::cout << "SSL_accept error: " << error << std::endl;
...
void Ipv4SslSocket::SSLLoadCertificates(const char *certFileName,
  ...
  status = SSL_CTX_use_certificate_file(
      static_cast<SSL_CTX *>(this->sslContext), certFileName, SSL_FILETYPE_PEM);
  ...
  if (status <= 0) {
    ...
    status =
        SSL_CTX_use_PrivateKey_file(static_cast<SSL_CTX *>(this->sslContext),
                                    keyFileName, SSL_FILETYPE_PEM);
