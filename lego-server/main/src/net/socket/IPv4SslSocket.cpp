...
IPv4SslSocket::IPv4SslSocket(int socketFD, const IPv4SslSocket *original) : socketFD(socketFD),
                                                                            ssl(original->sslContext),
                                                                            sslContext() {
  if (!SSL_set_fd(static_cast<SSL *>(ssl), socketFD)) {
    throw runtime_error(appendErr("IPv4SslSocket::IPv4SslSocket: Failed to create client socket: "));
  }
}
...
void IPv4SslSocket::connect(const string &host, const string &service) const {
  ...
  if (0 != st) {
    throw runtime_error("IPv4SslSocket::connect: Failed to get address info: " + string(gai_strerror(st)));
  }
...
string IPv4SslSocket::getCerts() const {
  ...
  char *subject = X509_NAME_oneline(X509_get_subject_name(peerCertificates), nullptr, 0);

  out << "Subject: " << subject << endl;

  char *issuer = X509_NAME_oneline(X509_get_issuer_name(peerCertificates), nullptr, 0);
...
int IPv4SslSocket::getSocketFD() const {
  return socketFD;
}
