...
/**
 * @brief Starts the listener, creates the threads and starts them running. Besides
...
void FigureHttpsServer::sendHttpResponse(const std::shared_ptr<IPv4SslSocket>& client, int statusCode,
                                         const std::map<std::string, std::string> &headers,
                                         const std::string &body) {
    string response = FigureHttpsServer::generateHttpResponse(statusCode, headers, body);
