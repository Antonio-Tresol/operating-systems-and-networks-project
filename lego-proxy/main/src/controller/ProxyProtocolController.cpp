...
ProxyProtocolController::ProxyProtocolController() {
  // this->availableFigures = std::make_shared<std::unordered_map<
  //     std::string, std::vector<std::string>>>();  // Don't delete
}
...
void ProxyProtocolController::handle(int code, std::string ip,
  ...
  //   // Fijarse si existe la figura en availableFigures, si no existe, se
  //   agrega. int indexNthOccurrence = findNthOccurrence(body, SEPARATOR, 2);
  ...
  //   string extractedBody = body.substr(
  //       indexNthOccurrence + 1);  // Se quita el codigo, separador e
  //       ip:puerto.
