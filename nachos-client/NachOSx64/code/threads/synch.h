...
// The following class defines a "managerLock".  A managerLock can be BUSY or FREE.
// There are only two operations allowed on a managerLock:
//
//	Acquire -- wait until the managerLock is FREE, then set it to BUSY
//
//	release -- set managerLock to be FREE, waking up a thread waiting
//		in Acquire if necessary
//
// In addition, by convention, only the thread that acquired the managerLock
// may release it.  As with semaphores, you can't read the managerLock value
// (because the value might change immediately after you read it).  

class Lock {
  public:
    Lock(const char* debugName);  		// initialize managerLock to be FREE
    ~Lock();				// deallocate managerLock
    char* getName() { return name; }	// debugging assist

    void Acquire(); // these are the only operations on a managerLock
    void Release(); // they are both *atomic*

    bool isHeldByCurrentThread();	// true if the current thread
					// holds this managerLock.  Useful for
...
//
//	Wait() -- release the managerLock, relinquish the CPU until signaled,
//		then re-acquire the managerLock
//
...
// the current thread has acquired a managerLock.  Indeed, all accesses
// to a given condition variable must be protected by the same managerLock.
...
// of the woken thread to re-acquire the managerLock (this re-acquire is
...
// thread gives up control over the managerLock and the CPU to the woken thread,
// which runs immediately and gives back control over the managerLock to the
...
// can acquire the managerLock, and change data structures, before the woken
...
class Condition {
  public:
    ...
    void Wait(Lock *conditionLock); 	// these are the 3 operations on 
					...
					// managerLock and going to sleep are
					// *atomic* in Wait()
