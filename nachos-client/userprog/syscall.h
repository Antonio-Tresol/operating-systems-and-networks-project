...
/* Only return once the the user program "spaceId" has finished.
...
typedef int Lock_t;
/* LckCreate creates a managerLock structure */
Lock_t LckCreate( );

/* LckDestroy destroy an already created Lock */
int LckDestroy( Lock_t lockId );

/* LckLock obtain the managerLock, if busy the thread must wait */
int LckLock( Lock_t lockId );

/* LckUnlock release the obtained managerLock freeing it to be used for others */
