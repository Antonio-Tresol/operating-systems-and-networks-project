...
ssize_t Socket::sendTo(const void *buffer /*message*/, size_t length, void *pDest_addr) {
    ...
    if (!ipv6) {
        sent = sendto(idSocket, buffer, length, 0, (const struct sockaddr *) pDest_addr,
...
bool Socket::isReadReady() {
    ...
    if (0 > select_result) {
        std::cerr << "IPv4SslSocket::isReadReady: Failed to select read_fds: " << std::endl;
        exit(2);
    }
