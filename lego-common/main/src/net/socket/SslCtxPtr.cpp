...
SslCtxPtr::SslCtxPtr() {  // TODO(aarevalo): Risky shit. Split IPv4SslSocket
                          // into two classes if this doesn't work.
  SSL_library_init();
  ...
  if (!method) {
    throw runtime_error(
        appendSslErr("SslCtxPtr::SslCtxPtr(): Failed to create SSL_METHOD: "));
  }

  ctx = SSL_CTX_new(method);
  if (!ctx) {
    throw runtime_error(
        appendSslErr("SslCtxPtr::SslCtxPtr(): Failed to create SSL_CTX: "));
  }
}
...
SslCtxPtr::SslCtxPtr(const std::string &certPath, const std::string &keyPath) {
  ...
  if (0 >=
    ...
    throw runtime_error(
        appendSslErr("SslCtxPtr::SslCtxPtr(): Failed to set certificate: "));
  }

  if (0 >=
      SSL_CTX_use_PrivateKey_file(ctx, keyPath.c_str(), SSL_FILETYPE_PEM)) {
    throw runtime_error(
        appendSslErr("SslCtxPtr::SslCtxPtr(): Failed to set private key: "));
  }

  if (!SSL_CTX_check_private_key(ctx)) {
    throw runtime_error(
        appendSslErr("SslCtxPtr::SslCtxPtr(): Failed to verify private key: "));
  }
}
